...
export default function Home() {
  ...
  return (
    <main ref={mainRef} style={{background:'#2E3A4A', minHeight:'100vh', fontFamily:'DM Sans, sans-serif', fontWeight:200, color:'#EDEBE5'}}>
      ...
      {/* HERO */}
      <section className="about-page-top" style={{position:'relative', width:'100%', height:'100vh', background:'#2E3A4A', overflow:'hidden'}}>
        <HeroImage src="/hero.jpg" alt="Razor Tech Archive" parallax priority />
        <div style={{position:'absolute', inset:0, background:'linear-gradient(to bottom, rgba(46,58,74,0) 55%, rgba(46,58,74,0.9) 100%)', zIndex:5, pointerEvents:'none'}}></div>
        <span className="hero-vert about-fade-up" style={{position:'absolute', left:24, top:'50%', transform:'translateY(-50%) rotate(180deg)', writingMode:'vertical-rl', fontSize:8, letterSpacing:'0.35em', textTransform:'uppercase', color:'rgba(237,235,229,0.15)', zIndex:10}}>Archive</span>
        ...
        <div className="hero-bottom about-fade-up" style={{position:'absolute', bottom:40, left:0, right:0, padding:'0 48px', display:'flex', alignItems:'flex-end', justifyContent:'flex-end', zIndex:10}}>
          <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:12}}>
            <span style={{fontFamily:"'DM Sans', sans-serif", fontSize:8, letterSpacing:'0.3em', textTransform:'uppercase', color:'rgba(237,235,229,0.5)', writingMode:'vertical-rl'}}>Scroll</span>
            <div style={{
              width:1,
              height:52,
      ...
      {/* TICKER */}
      <div className="about-fade-up" style={{borderTop:'1px solid rgba(237,235,229,0.15)', borderBottom:'1px solid rgba(237,235,229,0.15)', padding:'13px 0', overflow:'hidden'}}>
        <div style={{display:'flex', gap:60, animation:'tick 28s linear infinite', whiteSpace:'nowrap'}}>
          {['Razor Tech Archive','Razor Cut','Precision Has No Language','感覚を理論へ','From Sensation to Theory','Razor Tech Archive','Razor Cut','Precision Has No Language','感覚を理論へ','From Sensation to Theory'].map((t,i) => (
            <span key={i} style={{fontFamily:"'Hiragino Mincho Pro', 'ヒラギノ明朝 Pro', serif", fontSize:8, letterSpacing:'0.35em', textTransform:'uppercase', color:'rgba(237,235,229,0.72)', flexShrink:0}}>{t} &nbsp;✦&nbsp; </span>
          ))}
        </div>
      </div>

      {/* SEMINAR */}
      <section className="section-pad" style={{padding:'clamp(72px, 11vw, 140px) clamp(24px, 6vw, 80px)', borderTop:'1px solid rgba(237,235,229,0.15)'}}>
        ...
        {seminars.map((s) => (
          <a key={s.num} href="/seminar" className="seminar-item about-fade-up" style={{display:'grid', gridTemplateColumns:'180px 1fr auto auto', alignItems:'center', gap:48, padding:'36px 0', borderBottom:'1px solid rgba(237,235,229,0.15)', textDecoration:'none', color:'#EDEBE5'}}>
            <span style={{fontSize:10, color:'rgba(237,235,229,0.72)', letterSpacing:'0.12em'}}>{s.num} — {s.date}</span>
            <span className="seminar-name" style={{fontFamily:"'Hiragino Mincho Pro', 'ヒラギノ明朝 Pro', serif", fontSize:'clamp(18px,2.4vw,30px)', fontWeight:300, color:'#EDEBE5'}}>{s.name}</span>
            <span style={{fontFamily:"'Hiragino Mincho Pro', 'ヒラギノ明朝 Pro', serif", fontSize:10, color:'rgba(237,235,229,0.72)', letterSpacing:'0.08em'}}>{s.loc}</span>
      ...
      {/* MEMBERS */}
      <section className="section-pad" style={{padding:'clamp(72px, 11vw, 140px) clamp(24px, 6vw, 80px)', borderTop:'1px solid rgba(237,235,229,0.15)'}}>
        <div className="members-banner" style={{border:'1px solid rgba(237,235,229,0.15)', padding:'clamp(56px, 8vw, 100px) clamp(28px, 6vw, 80px)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:60, position:'relative', overflow:'hidden'}}>
          <div>
          ...
          <a href="/members" className="about-fade-up" style={{fontSize:9, letterSpacing:'0.28em', textTransform:'uppercase', color:'#EDEBE5', border:'1px solid rgba(237,235,229,0.15)', padding:'20px 52px', textDecoration:'none', whiteSpace:'nowrap', flexShrink:0}}>Join Now</a>
        </div>
      </section>
